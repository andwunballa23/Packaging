...
function Install-ADTDeployment
{
    ...
    Start-ADTMsiProcess -Action Install `
        -FilePath "$dirFiles\Totalflow Simulator (G4)\Setup.msi" `
        -AdditionalArgumentList "/qn /norestart /l*v `"C:\distrib\logs\TotalflowSimulatorG4_Install.log`""

    Start-ADTMsiProcess -Action Install `
        -FilePath "$dirFiles\Totalflow Simulator (NGC)\Setup.msi" `
        -AdditionalArgumentList "/qn /norestart /l*v `"C:\distrib\logs\TotalflowSimulatorNGC_Install.log`""

    # Run main installer (InstallShield)
    Start-ADTProcess `
        -FilePath "$dirFiles\Setup.exe" `
        -ArgumentList "/S /f1`"$dirFiles\Setup.iss`" /f2`"C:\distrib\logs\PCCU7.58_Install.log`""
...
function Uninstall-ADTDeployment
{
    ...
    foreach ($app in $apps) {
        ...
        Start-ADTProcess -FilePath "cmd.exe" -ArgumentList "/c $($app.Command) >> `"$logFile`" 2>&1" -WindowStyle Hidden
